...
"""
Cheat Codes
moonwalk = reverse zombie movement
johnwick = infinite ammo
...
class App():
    def __init__(self):
        ...
        self.saveName = ""
        self.saving = 0
        self.cheatCodes = ["moonwalk", "johnwick", "invulnerable"]
        self.cheats = None
        self.mouseX = 0
        self.mouseY = 0
...
class cheatWindow:
    def __init__(self):
        ...
        self.stringVar = StringVar(self.top)
        self.stringVar.set("")
        self.entry = Entry(self.top, textvariable=self.stringVar).grid(row=0, column=0)

        self.button = Button(self.top, text="Enter", command=self.quit).grid(row=0, column=1)

        self.return1 = str()

    def quit(self):
        if self.stringVar.get() in app.cheatCodes:
            self.return1 = self.stringVar.get()
            app.addCheat(self.return1)
            self.top.destroy()
        else:
